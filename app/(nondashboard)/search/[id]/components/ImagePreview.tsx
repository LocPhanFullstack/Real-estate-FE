...
const ImagePreview = ({ images }: ImagePreviewsProps) => {
  ...
  return (
    <div className="relative h-112.5 w-full">
      ...
      <button
        onClick={handlePrev}
        className="absolute left-0 top-1/2 cursor-pointer transform -translate-y-1/2 bg-primary-700 bg-opacity-50 p-2 rounded-full focus:outline-none"
      ...
      <button
        onClick={handleNext}
        className="absolute right-0 top-1/2 cursor-pointer transform -translate-y-1/2 bg-primary-700 bg-opacity-50 p-2 rounded-full focus:outline-none"
        aria-label="Previous image"
      >
